...
public class CourseList extends AppCompatActivity {
    ...
    private final String URL = Server.ip + "getcourses.php";
    ...
    private void getInfos() {
        class Async extends AsyncTask<String, Void, String> {
            ...
            protected void onPreExecute() {
                super.onPreExecute();
                loadingDialog = ProgressDialog.show(CourseList.this, "please waite...", "Connecting....");
            }
                ...
                if (result.isEmpty() || result.equals("Error"))
                    ...
                    try {
                        ...
                        for (int i = 0; i < allReq.length(); i++) {
                            ...
                            temp.setPost_id(row.getString("course_id"));
                            temp.setTitle(row.getString("title"));
                            temp.setDetails(row.getString("details"));
                            temp.setPlace(row.getString("place"));
                            temp.setPeriod(row.getString("period"));
                            temp.setStart(row.getString("start_date"));
                            temp.setAdd_date(row.getString("add_date"));
                            temp.setContact(row.getString("contact_details"));
                            posts.add(temp);
